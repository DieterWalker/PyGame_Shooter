...
class Player(pygame.sprite.Sprite):
    ...
    def send_coordinates_event(self):
        send_sprite =  self.sprite_name + "_" + self.direction
        pygame.event.post(pygame.event.Event(pygame.USEREVENT, {'x': self.rect.x, 'y': self.rect.y,'sprite': send_sprite}))

    # Cập nhật Sprite
    def update_sprite(self):
        # Kiểm tra trạng thái của nhân vật để chọn sprite phù hợp
        if self.y_vel < 0:
            self.sprite_name = "jump"
        elif self.x_vel != 0:
            self.sprite_name = "run"
        elif self.x_vel == 0 and self.y_vel == 0:
            self.sprite_name = "idle"
        
        self.send_coordinates_event()

        sprite_sheet_name = self.sprite_name + "_" + self.direction
